...
addition to that, each subtree is in general expected to be induced by
its root node (i.e. `S := T[r]`, i.e. `S := T[OS(r)]`).
...
in `T`. (This obviously only applies to ordered trees).
...
subtree to both and not a forest of two or more subtrees or a non-tree graph.
...
Note that these operations return boolean values.
...
operator. This definition requires one input graph to be not just a subgraph,
but a subtree of the other.
...
of a union of trees needs to be a single tree.
...
have a non-empty intersection and are unrelated with each other).

Case-1/2: The result in these cases will be one of the input trees. That is
because a subtree can not add a new element to its super-tree (it would not
be a subtree, if it had an element outside of its super-tree).
...
a node in `S`, (Case-5) root `s` is not a node in `T`, and (Case-6) `s` is a
node in `T` and/or `t` a node in `S`.
...
the root of one tree must be a node in the other.
...
because in case of `(t ancestor-of s)`, new subtrees will be added to one
or more nodes in `T`.
...
Note that, due to its definition (i.e. remove elements of the 2nd tree from
the 1st tree) this operation is **not commutative**. That is, the order of
both operands is relevant.
...
with each other. And because `T` can be understood to be restricted the common
subtree, **T is expected to be a subtree of S**. The root of `T` is therefore
...
is guaranteed to have a leaf `l` that is a node, but no leaf in `S`. Because
of that, the result of `(S \ T)` will contain an induced subtree for each
child of `l` (i.e. `S[c]` where `(c child-of l)`). Consequently, the result
is in general a forest of one or more trees.
...
rather than being true in a very particular case.
