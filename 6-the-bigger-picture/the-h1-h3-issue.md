...
Note that the "rank" property of a section object does not actually exist.
These properties act as a visual representation of the section's outline depth.
...
such a section. Likewise, WC can not add "C" as subsection to "A" because both
rank values don't add up (i.e. `(A.rank != C.rank-1)`). That is, the combination
of both rank values are non-conformant under that pedantic perspective.
...
WC can only cancel indexing the document. And, as a consequence, no one will be
able to find that document by the use of WC's search engine.
...
is not accurate and only a best-effort approximation.
...
initial fragment because that fragment has no `h2` element. Similar to the
result of option 3, the result of this option is a best-effort approximation.

**Option 3 or 4:** But which one?

Because fragment-1 holds an input error, WC is not in the position to produce an
accurate result for fragment-1. But, as WC has to choose an option in order to
produce a best-effort result, and provided that there is no other option, WC can
only choose between option 3 or 4.

So here is the thing: Can WC freely choose an option? No, it can't, because
both of will return different results. Although any result is better than no
result at all, being able to freely choose which result to return under these
kind of circumstances would be just as bad: The very same document would end
up getting indexed based on someone's personal preference.

Note that HTML 3.2, 4.01, 5.0, 5.1 and 5.2 do not define heading content
elements to hold absolute rank values. They always are expressed as relative
...
to return. This would yield different results for the same content. There must
be an agreement on what the best-effort result has to be.
...
which results those other fragments support. That is, if they support only one
solution, then that one solution is what the result has to be. But if a fragment
offers multiple approximations, then an agreement is needed on which result to
return for that fragment.
...
Note that I consider, if no explicit rank is given, section elements to
...
WC can only add "B" as subsection to "A" and set "B"'s rank value to 2:
...
which is why WC has to skip that line.
...
re-used for the definition of the section's title. WC must therefore change
"C"'s title to "C":
...
by the `h3` element does have an internal effect: If that section element had
...
that fragment did not specify a title for "B". WC has no other option than to
leave "B" as is.
...
fit into the fragment's logical hierarchy. And because of that, a low-level
implementation can (and even has to) resolve the fragment's input error with
a commonly accepted approximation.

*fragment-2/toc-2:*
The "issue" with this fragment is based upon multiple elements, which combined
do not even represent an input error. Because of that, an implementation can
and must return the only result possible. As bad-practice as it may be, toc-2
accurately represents the fragment's contents.
...
bad-practice will become part of the output. That is not a choice, this is
...
about it. So yes, not specifying a heading can and should still be declared as
bad practice.
...
* What is supposed to happen in case of whole hierarchies of "empty" sections
  that have sparse, deeply-nested but non-empty inner subsections?
...
In other words: There are many questions that will have to be answered
in order to clearly define how to deal with seemingly empty sections.
...
accessed via an in-document path A and assumed that the in-document path the
user can follow is path B. Obviously, it won't help much, if both path values
(A and B) turn out to be substantially different.
