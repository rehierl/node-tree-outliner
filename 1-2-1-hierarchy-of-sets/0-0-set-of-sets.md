...
set as one of its elements. Obviously, the focus here is on a set that has
another set as subset (i.e. "subset-of" vs. "element-of").
...
Sets A and B differ in size and are therefore distinct. Likewise, sets B
...
(i.e. inside vs. outside, aka. subset vs. superset) between the two. The
...
In order to avoid such an unclear order between two sets, each set needs to
have elements which allow to distinguish a set from all the other sets. That
is, sets **must be distinct** from one another, which is why a set-of-sets,
...
As before, there is no distinct order between both sets.
...
since both sets have not even one element in common.

* `(V disjoint-to W) := ((V & W) == {})`
* `(V disjoint-to W) <-> (W disjoint-to V)`

Note that the empty set `{}` is the only set that is disjoint to itself.
...
one another. That is because otherwise, both subsets could not be subsets
of their respective supersets.
...
* `(V disjoint-to W) ex-or (V coupled-with W)`
* `(disjoint ex-or coupled) <=> (disjoint <-> !coupled)`

Note that the empty set `{}` is the only set that is not coupled with
...
* `overlap(V,W) := ((V & W) != Ø) and ((V \ W) != Ø) and ((W \ V) != Ø)`
* e.g. `overlap(A,D) == [ ({2} != Ø) and ({1} != Ø) and ({3} != Ø) ]`
...
<!-- ======================================================================= -->
## (simple) subset, strict/proper subset
...
However, no set (including `{}`) is a strict subset to itself.

Note that the empty set `{}` is a subset to any (non-empty) set.
In contrary to that, no non-empty set is a subset of the empty set.

Note that the empty set `{}` is a strict subset to any non-empty set.
...
Note that the attempt to support the empty set `{}` as an element, will
add additional difficulty when classifying the relationship between two
sets. That is due to the unique characteristics of the empty set.
