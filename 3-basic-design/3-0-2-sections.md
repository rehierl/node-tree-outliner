...
Note that the direction of the `contains` relation is top-down: A superordinate
entity contains an entity which is subordinate to it. Obviously, the direction
of the inverted `element-of` relation is bottom-up.

**DEFINITION**
A section is a sequence of nodes.

When the algorithm traverses the node tree, it adds each node to one or more
sections, one after another, in a specific order. Because of that, a section
has a first and a last node. Consequently, sections can also be defined as
node sequences:
...
sections. Rules are therefore required in order to uniquely determine a node's
location with regards to the sections of its surrounding tree.
...
subordinate section. That is, the `declares` relation is top-down, and the
`declared-by` relation is bottom-up.
...
have an effect on itself as this information would be used to set a property
...
or some node property), then the set of active nodes will also contain the
corresponding nodes.
