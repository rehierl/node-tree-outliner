...
**CLARIFICATION**
Any descendant of a node, that is associated with a section, loosely belongs
to the same section (downwards). Also, any node loosely belongs to any section
with which any of its ancestors is associated (upwards).

That is, because a semantically consistent path can be defined which connects
a section with any of the associated node's descendants.
...
"descendant" is also based upon the construction of a semantically consistent
path. In addition to that, the term "descendant" is used as the basis for the
definition of the term "ancestor".
...
to produce a conflict, because any descendant of an associated container
loosely belongs to the container's sections.

Again these loose associations are established automatically by the structure
of a given tree and can therefore not be undefined.
...
section `s`, if the distinction between a "strict" and a "loose" relationship
is not relevant. That is, if a loose association can be seen to be equal to a
strict association.

From this point on, all associations (strict/explicit or loose/implicit) are
considered to be semantically equivalent. Because of that, this semantic
equality is itself a necessity/requirement.
...
associated with it or not. Because of that, section `s` does not end inside of
node `C`.
...
**CLARIFICATION**
Any section is defined by a sequence of nodes which only contains the section's
top-level nodes (in the order of the tree's node sequence). Any other node,
which is related to that section, can be derived from the section's sequence of
top-level nodes.
...
* If a sequence is created by removing one or more nodes from
  a reduced sequence, then that sequence is insufficient to determine
  the section's complete sequence.
* The reduced sequence represents the shortest sequence possible which
  still allows to determine the complete sequence.
...
In between a section's open and closed states, any subsequent node must be
...
section did not end with the first node, then the next node that must be
...
for the first node itself, all are subsequent siblings of the section's first
node, or of any of its ancestors.

In addition to that, the reduced sequence begins with the section's first node
that is followed by its siblings, which are followed by the siblings of the
first node's parent. After that, the sequence continues with the siblings of
the first node's next upper ancestor until it finally ends with the siblings
of the last relevant ancestor.

**Memory hook**
The reduced sequence of a section, or even a section itself, could loosely be
...
ancestors. Descendants are implicitly associated with these by the node tree's
structure.
...
**CLARIFICATION**
A non-empty section can be seen to end with an unrelated container (i.e.
...
**CLARIFICATION**
Unlike a section's first node, it is in general not possible to clearly identify
the last node of a section. Obviously, the same also applies to the relationship
...
define a section's last node without enforcing a certain structure upon the
nodes of a section. If possible, the latter must obviously be avoided.
