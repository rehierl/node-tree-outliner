...
of nodes. Consequently, operators defined for sets can be used on sections:
...
By itself, the `add` operation defines a many-to-many (N:M) relation.
...
Consequently, an outline algorithm must traverse the node tree in order to visit
...
order. Because of that order, sections can also be defined as a sequences of
nodes:
...
Both of the above definitions allow to associate any node with multiple sections.
Additional rules are therefore required to uniquely determine the effective
context of a node.
...
`ns` of the corresponding tree of nodes:
...
subsequent nodes will not be entered or exited, while a presequent node is
still in the process of being entered or exited.
...
Such an algorithm has, when entering its current node `ni`, knowledge of any
node within subsequence `s1 := [n1,ni]`. The algorithm's current knowledge
...
presequent ancestor `ni`. That is because any process will have knowledge of any
...
any operation that might have some side effect on any node within `s1`.
Operations that depend on this kind of delayed knowledge, have the potential to
...
exited (i.e. they can affect nodes in `s2`, but must not have any side effect
on any node in `s1`).
...
entered. Because of that, any new section must be added to the set of sections
`S` as soon as its sectioning node is entered. With that in mind, `S` can be
seen to represent the set of known sections.

In general, and from that point on, any subsequent node must be associated with
such a new and any other known section (i.e. any section in `S`). Consequently,
any node potentially belongs to multiple sections because any node can be
subsequent to any number of sectioning nodes.
